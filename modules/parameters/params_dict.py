...
class ParamsDict(object):
    ...
    def override(self, override_params, is_strict=True):
        ...
        self._override(override_params,
                       is_strict)  # pylint: disable=protected-access
            ...
            if k not in self.__dict__.keys():
                ...
                if isinstance(v, dict):
                    self.__dict__[k]._override(
                        v, is_strict)  # pylint: disable=protected-access
                elif isinstance(v, ParamsDict):
                    self.__dict__[k]._override(
                        v.as_dict(), is_strict)  # pylint: disable=protected-access
                else:
                    self.__dict__[k] = copy.deepcopy(v)
    ...
    if isinstance(dict_or_string_or_yaml_file, dict):
        ...
        if isinstance(params_dict, dict):
            ...
            with tf.io.gfile.GFile(dict_or_string_or_yaml_file) as f:
                params.override(
                    yaml.load(f, Loader=yaml.FullLoader), is_strict)
    else:
