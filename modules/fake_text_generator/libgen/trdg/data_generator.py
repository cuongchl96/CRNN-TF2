...
from modules.fake_text_generator.libgen.trdg import computer_text_generator, background_generator, distorsion_generator

try:
    from modules.fake_text_generator.libgen.trdg import handwritten_text_generator
...
class FakeTextDataGenerator(object):
    ...
    def generate(
        ...
        size = random.randint(min(40, max_size), max_size)
        if is_handwritten:
            if orientation == 1:
        ...
        #############################
        if random.random() < .4:
            distorsion_type = random.randint(0, 0)
        ...
        #############################
        background_type = random.randint(3, 3)
        ...
        if random.random() < .5:
            r = rnd.randint(0, blur)
            gaussian_filter = ImageFilter.GaussianBlur(
                radius=blur if not random_blur else r
            )
